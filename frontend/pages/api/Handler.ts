...
import { contractABI, contractAddress, loandiskAPIBaseURL, loandiskAuthCode } from '../utils/constants';
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    if (transactionType === 'deposit') {
      transactionResponse = await contract.deposit(ethers.utils.parseEther(amount), { from: borrowerId });
    } else {
      transactionResponse = await contract.withdraw(ethers.utils.parseEther(amount), { from: borrowerId });
    }
    ...
    const response = await axios.post(`${loandiskAPIBaseURL}/transactions`, loanDiskTransaction, {
      headers: {
